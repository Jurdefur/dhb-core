DHBcoin Core version 0.18.1 is now available from:

  <https://download.dhbcoin.org/dhbcoin-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/dhbcoin-dev>
...
`/Applications/DHBcoin-Qt` (on Mac) or `dhbcoind`/`dhbcoin-qt` (on
Linux).
...
DHBcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use DHBcoin Core on unsupported systems.

DHBcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, DHBcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@dhbcoin.org) the DHBcoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 DHB),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in DHB per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/dhbcoin-conf.md)
  about the `dhbcoin.conf` file describes how to use it to configure
  DHBcoin Core.

- A new document introduces DHBcoin Core's BIP174 [Partially-Signed
  DHBcoin Transactions
  (PSBT)](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent DHBcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  DHBcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/dhbcoin-project/dhbcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when DHBcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of DHBcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/dhbcoin-project/dhbcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart dhbcoind with the `-deprecatedrpc=generate`
...
- A new `dhbcoin-wallet` tool is now distributed alongside DHBcoin
...
This section describes planned changes to DHBcoin Core that may affect
other DHBcoin software and services.

- Since version 0.16.0, DHBcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with DHBcoin Core 0.20 (expected about
  a year after 0.18), DHBcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the DHBcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in DHBcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any DHBcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, DHBcoin Core
...
- A sub-project of DHBcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with DHBcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to DHBcoin Core's own implementation, although entropy
  gathered by DHBcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves DHBcoin Core a
...
- On macOS, DHBcoin Core now opts out of application CPU throttling
