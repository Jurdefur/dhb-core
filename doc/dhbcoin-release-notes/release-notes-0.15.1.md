DHBcoin Core version *0.15.1* is now available from:

  <https://download.dhbcoin.org/dhbcoin-0.15.1/>
...
A number of changes to the way DHBcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 knaak per kB (0.00001 DHB). This is to prepare for dropping the minimum transaction fee to 0.1 knaak per kB (0.0001 DHB) in the next release.
